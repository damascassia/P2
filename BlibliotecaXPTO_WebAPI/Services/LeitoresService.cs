...
namespace BlibliotecaXPTO_WebAPI.Services
{
    public class LeitoresService : ILeitoresService
    {
        ...
        public void CancelarInscricao(string numeroDocumento)
        {
            try
            {
                _logger.LogInformation($"Cancelar inscrição: numeroDocumento={numeroDocumento}");
                _repo.CancelarInscricao(numeroDocumento, _activeTag);
            }
            catch { throw; }
        }
    }
